...
import { FoodRecordCard, EmptyState } from "@/components/ui/molecules";
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        {records.length > 0 && (
          <button className="text-sm text-green-600 hover:text-green-700 font-medium">전체 보기</button>
        )}
      </div>
      ...
      {records.length === 0 ? (
        ...
        <div className="space-y-3">
          {records.map((record) => (
            <FoodRecordCard key={record.id} record={record} onClick={onRecordClick} variant={variant} />
          ))}
        </div>
      )}
    </div>
